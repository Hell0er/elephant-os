...
/* 安装一页大小的vaddr，专门针对fork虚拟地址位图无须操作的情况 */
...
void* sys_malloc(uint32_t size){
    ...
    if (cur_thread->pgdir==NULL){   // 若为内核线程
        ...
        descs=k_block_descs;
    }else {   // 若为用户进程，其pcb中的pgdir会在分配页表时创建
        pf=PF_USER;
    ...
    if (size>1024){
    ...
    }else {   // 若申请的内存小于等于1024，可在各种规格的mem_block_desc中适配
        uint8_t desc_idx;

        /* 从内存块描述符中匹配合适的内存块规格 */
        for (desc_idx=0;desc_idx<DESC_CNT;++desc_idx){
            if (size<=descs[desc_idx].block_size){   // 从小往大依次寻找，找到第一个比所需大的即可退出
                break;
            }
	    }
        ...
        * 就创建新的arena提供mem_block */
        if (list_empty(&descs[desc_idx].free_list)){
            a=malloc_page(pf,1);   // 分配1页页框作arena
            if (a==NULL){
                lock_release(&mem_pool->lock);
                return NULL;
            }
            ...
            * cnt置为此arena可用的内存块数，large置为false */
            ...
            /* 开始将arena拆开成内存块，并添加到内存块描述符的free_list中 */
    	...
    	a=block2arena(b);   // 获取内存块b最在的arena
    	--a->cnt;           // 将此arena中的空闲内存块-1
    	lock_release(&mem_pool->lock);
    	return (void*)b;
    }
}
...
static void page_table_pte_remove(uint32_t vaddr){
    uint32_t* pte=pte_ptr(vaddr);
    *pte&=~PG_P_1;                               // 将页表项pte位置0
    asm volatile("invlpg %0"::"m"(vaddr):"memory");// 更新tlb
}

/* 在虚拟地址池中释放_vaddr起始的连续pg_cnt个虚拟地址 */
...
void mfree_page(enum pool_flags pf,void* _vaddr,uint32_t pg_cnt){
    ...
    /* 判断pg_phy_addr属于用户物理内存池还是内核五i了内存池 */
...
void free_a_phy_page(uint32_t pg_phy_addr){
    ...
    if (pg_phy_addr>=user_pool.phy_addr_start){
        mem_pool=&user_pool;
        bit_idx=(pg_phy_addr-user_pool.phy_addr_start)/PG_SIZE;   // 获取位图索引
    }else {
        mem_pool=&kernel_pool;
        bit_idx=(pg_phy_addr-kernel_pool.phy_addr_start)/PG_SIZE;
    }
    bitmap_set(&mem_pool->pool_bitmap,bit_idx,0);   // 物理内存池中的相应位置0
}
