...
static void release_prog_resource(struct task_struct* release_thread){
    ...
    uint32_t pte=0;
    uint32_t* v_pte_ptr=NULL;   // 和pte_ptr区分

    uint32_t* first_pte_vaddr_in_pde=NULL;   // 用于记录pde中第0个pte的地址
    ...
    while (pde_idx<user_pde_nr){
		...
		pde=*v_pde_ptr;
		if (pde & 0x00000001){   // 如果页目录项p位为1，表示该目录项下可能有页表
			first_pte_vaddr_in_pde=pte_ptr(pde_idx * 0x400000);   // 一个页表为4M
			pte_idx=0;
			while (pte_idx<user_pte_nr){
				v_pte_ptr=first_pte_vaddr_in_pde+pte_idx;
				pte=*v_pte_ptr;
				if (pte & 0x00000001){
					/* 将pte中记录的物理页框直接在相应的内存池的位图中清0 */
...
/* list_traversal的回调函数，
 * 查找pelem的parent_pid是否是ppid，成功则返回true，失败则返回false */
...
static bool init_adopt_a_child(struct list_elem* pelem,int32_t pid){
    ...
    if (pthread->parent_pid==pid){
		pthread->parent_pid=1;   // init进程的pid=1
    }
    return false;
}
    ...
    while (1){
		...
		if (child_elem!=NULL){
			...
			*status=child_thread->exit_status;
			...
			/* thread_exit之后，pcb被回收，因此提前获取pid */
			uint16_t child_pid=child_thread->pid;

			/* 2.从就绪队列和全部队列中删除进程表项 */
			thread_exit(child_thread,false);
			/* 进程表项是进程或线程的最后保留的资源，至此该进程彻底消失 */
		...
		if (child_elem==NULL){   // 若没有子进程则出错返回
			...
			/* 若子进程还未运行完，即还未调用exit()，则挂起 */
			thread_block(TASK_WAITING);   // 等待子进程执行exit()唤醒自己
		}
    }
}
