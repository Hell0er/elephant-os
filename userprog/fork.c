...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct* child_thread,struct task_struct* parent_thread){
    /* 1.复制pcb所在的整个页，里面包含进程pcb信息及0级栈，包含返回地址，再单独修改部分信息 */
    memcpy(child_thread,parent_thread,PG_SIZE);
    child_thread->pid=fork_pid();
    child_thread->elapsed_ticks=0;
    child_thread->status=TASK_READY;
    child_thread->ticks=child_thread->priority;   // 为新进程初始化时间片
     ...
     * 下面将child_thread->userprog_vaddr.vaddr_bitmap.bits指向自己的位图vaddr_bimp*/
    memcpy(vaddr_btmp,child_thread->userprog_vaddr.vaddr_bitmap.bits,bitmap_pg_cnt*PG_SIZE);
    child_thread->userprog_vaddr.vaddr_bitmap.bits=vaddr_btmp;
    /* 调试用 */
    ASSERT(strlen(child_thread->name)<11);   // pcb.name的长度为16，为避免下面strcat越界
        ...
        if (vaddr_btmp[idx_byte]){
            ...
            while (idx_bit<8){
                if ((BITMAP_MASK<<idx_bit)&vaddr_btmp[idx_byte]){
                    ...
                    /* 2.1 将父进程在用户空间中的数据复制到内核缓冲区buf_page，
                    * 目的是下面切换到子进程的页表后，还能访问到父进程的数据 */
                    memcpy(buf_page,(void*)prog_vaddr,PG_SIZE);

                    /* 2.2 将页表切换到子进程，目的是避免下面申请内存的函数将pte及pde安装在父进程的页表中 */
                    page_dir_activate(child_thread);
                    /* 2.3 申请虚拟地址prog_vaddr */
                    get_a_page_without_opvaddrbitmap(PF_USER,prog_vaddr);

                    /* 2.4 从内核缓冲区中将父进程数据复制到子进程的用户空间 */
...
static int32_t build_child_stack(struct task_struct* child_thread){
    ...
    /* 2.为switch_to构建struct thread_stack，将其构建在紧邻intr_stack之下的空间 */
    ...
    /* ebp在thread_stack中的地址便是当时esp（0级栈栈顶），
     * 即esp为（uint32_t*)intr_0_stack-5 */
    uint32_t* ebp_ptr_in_thread_stack=(uint32_t*)intr_0_stack-5;

    /* switch_to的返回地址更新为intr_exit，直接重中断返回 */
    *ret_addr_in_thread_stack=(uint32_t)intr_exit;

    /* 下面这两行复制只是为了使构建的thread_stack更加清晰，其实也不需要，
