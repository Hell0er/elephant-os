...
/* 在分区中找到名为part_name的分区，并将其指针赋给cur_part */
...
static void partition_format(struct partition* part){
    ...
    sb.block_bitmap_lba=sb.part_lba_base+2;	// 第0块为bootblock，第1块为superblock
    ...
    uint32_t last_size=SECTOR_SIZE-(block_bitmap_last_byte%SECTOR_SIZE);   // last_size是位图中的填充部分

    /* 2.1先将位图最后一字节到其所在扇区结束全置为1，即超出实际块数部分直接置为已占用 */
    memset(&buf[block_bitmap_last_byte],0xff,last_size);

    /* 2.2再将上一步中覆盖的最后一字节内有效位重新置0 */
    uint8_t bit_idx=0;
    ...
    /* 由于inode_table中共4096个inode，位图中inode_bitmap正好占用1扇区，
...
char* path_parse(char* pathname,char* name_store){
    ...
    if (pathname[0]==0){   // 路径字符串为空则返回NULL
	    return NULL;
    }
    return pathname;
}
...
int32_t path_depth_cnt(char* pathname){
    ...
    char* p=pathname;
    char name[MAX_FILE_NAME_LEN];   // 用于path_parse路径解析
    uint32_t depth=0;
...
static int search_file(const char* pathname,struct path_search_record* searched_record){
    ...
    while (name[0]){   // 若第一个字符就是结束符，结束循环
        /* 记录查找过的路径，但不能超过searched_path的长度512Byte */
...
int32_t sys_open(const char* pathname,uint8_t flags){
    ...
    if (pathname_depth!=path_searched_depth){   // 说明并没有访问到全部路径，某个中间目录是不存在的
...
int32_t sys_lseek(int32_t fd,int32_t offset,uint8_t whence){
    ...
    switch(whence){
        ...
        /* SEEK_END 新的读写位置是相对于文件尺寸增加offset个位移量 */
        case SEEK_END:
...
int32_t sys_mkdir(const char* pathname){
    ...
    if (inode_no!=-1){
        ...
        rollback_step=1;
        goto rollback;
    }else {   // 若未找到，也要判断是在最终目录没找还是某个中间目录没找到
        uint32_t pathname_depth=path_depth_cnt((char*)pathname);
        uint32_t path_searched_depth=path_depth_cnt(searched_record.searched_path);
        /* 先判断是否把pathname的各层目录都访问到了，即是否在某个中间目录就失败了 */
        if (pathname_depth!=path_searched_depth){   // 说明没有访问到全部路径，即某个中间目录不存在
    ...
    /* 创建文件或目录需要创建相关的多个资源，若某步失败则hi执行到下面的回滚步骤 */
rollback:   // 回滚
...
int32_t sys_rmdir(const char* pathname){
    ...
    if (inode_no==-1){
        ...
        if (searched_record.file_type==FT_REGULAR){
            ...
            if (!dir_is_empty(dir)){
                printk("dir %s is not empty, it is not allowed to delete noempty directory!\n",pathname);
            }else {
...
char* sys_getcwd(char* buf,uint32_t size){
    ...
    memset(buf,0,size);
    char full_path_reverse[MAX_PATH_LEN]={0};   // 用来作全路径缓冲区
...
void filesys_init(){
    ...
    while (channel_no<channel_cnt){
        ...
        while (dev_no<2){
            ...
            while (part_idx<12){    // 四个主分区+八个逻辑分区
                if (part_idx==4){   // 逻辑分区
                    part=hd->logic_parts;
                }
                ...
                * 下面处理存在的分区.  */
                if (part->sec_cnt!=0){   // 如果分区存在
                    ...
                    /* 读出分区超级块，根据魔数是否正确来判断是否存在fs */
                    ide_read(hd,part->start_lba+1,sb_buf,1);

                    /* 只支持自己的文件系统，若磁盘上已经有fs就不再初始化了 */
