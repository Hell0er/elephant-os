...
int32_t pcb_fd_install(int32_t global_fd_idx){
    ...
    uint8_t local_fd_idx=3;   // 跳过stdin,stdout,stderr
    ...
    if (local_fd_idx==MAX_FILES_OPEN_PER_PROC){
		printk("exceed max open_per_proc\n");
		return -1;
    }
    return local_fd_idx;
}
...
/* 将内存bitmap第bit_idx位所在的512字节同步到硬盘 */
...
int32_t file_create(struct dir* parent_dir,char* filename,uint8_t flag){
    ...
    uint8_t rollback_step=0;   // 用于操作失败时回滚各资源
    ...
    /* 此inode要从堆中申请内存，不可生成局部变量（否则函数退出时会释放）
...
/* 打开编号为inode_no的inode对应的问价，若成功则返回文件描述符，否则返回-1 */
int32_t file_open(uint32_t inode_no,uint8_t flag){
    ...
    file_table[fd_idx].fd_pos=0;   // 使文件指针指向开头
    file_table[fd_idx].fd_flag=flag;
    bool* write_deny=&file_table[fd_idx].fd_inode->write_deny;

    if (flag & O_WRONLY || flag & O_RDWR){   // 可写
...
/* 把buf中的count字节写入file，成功则返回写入的字节数，失败则返回-1 */
int32_t file_write(struct file* file,const void* buf,uint32_t count){
    if ((file->fd_inode->i_size+count)>(BLOCK_SIZE*140)){
		printk("exceed max file_size 71680B, write file failed\n");
		return -1;
    }
    ...
    /* 开始将文件所以块地址收集到all_blocks，（系统中块大小等于扇区大小）
    ...
    if (add_blocks==0){
		...
		/* 若有增量，便涉及到分配新扇区是否分配一级间接块表，下面分三种情况：
		* 1. 12个直接块够用 */
		if (file_will_use_blocks<=12){
			/* 先将剩余空间可继续用的扇区地址写入all_blocks */
			block_idx=file_has_used_blocks-1;
			ASSERT(file->fd_inode->i_sectors[block_idx]!=0)
			all_blocks[block_idx]=file->fd_inode->i_sectors[block_idx];
			
			/* 再将未来要用的扇区分配号后写入all_blocks */
			...
			while (block_idx<file_will_use_blocks){
				...
				/* 写文件时，不应该存在块未使用但已分配扇区的情况，当文件删除时，就会把块地址清0 */
...
int32_t file_read(struct file* file,void* buf,uint32_t count){
    ...
    uint32_t block_read_end_idx=(file->fd_pos+size)/BLOCK_SIZE;   // 终止地址
    uint32_t read_blocks=block_read_start_idx-block_read_end_idx;   // 增量为0，表示在同一扇区
    ASSERT(block_read_start_idx<139 && block_read_end_idx<139);

    int32_t indirect_block_table;   // 用来获取一级间接表地址
    uint32_t block_idx;   // 获取待读的块地址

    /* 以下开始构建all_blocks块地址数组，存储用到的块地址 */
    ...
    while (bytes_read<size){
		...
		sec_left_bytes=BLOCK_SIZE-sec_off_bytes;
		chunk_size=size_left<sec_left_bytes?size_left:sec_left_bytes;   // 待读入
