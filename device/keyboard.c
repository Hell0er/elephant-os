...
static void intr_keyboard_handler(void){
    ...
    if(break_code){   // 若是断码（按键弹起时产生的扫描码）

    	uint16_t make_code=(scancode &= 0xff7f); //多字节扫描码暂不处理

	/* 若是以下三个键弹起了，将状态设为false */
    ...
    /* 若为通码，只处理数组中定义的键以及alt_right和ctrl键，全是make_code */
    	...
    	bool shift=false;   // 判断是否与shift结合，用来在一维数组中索引对应的字符，先默认设置成false
		...
		shift=true;
    	    ...
    	    if (shift_down_last && caps_lock_last){   // 同时按下shift和capslock
				shift=false;
	    	}else if (shift_down_last || caps_lock_last){   // 按下shift或capslock
				shift=true;
	    	}else{
				shift=false;
	    	}
