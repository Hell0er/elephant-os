...
#define reg_rerror(channel)	(channel->port_base+1)
...
#define reg_alt_status(channel)	(channel->port_base+0x26)
...
/* 打印分区信息 */
static bool partition_info(struct list_elem* pelem){
...
void ide_init(){
    ...
    uint8_t hd_cnt=*((uint8_t*)(0x475));	// 在地址0x475处可获取硬盘数量
    ASSERT(hd_cnt>0);
    ...
    while (channel_no<channel_cnt){
        ...
        switch (channel_no){
            ...
            channel->irq_no=0x20+14;   // ide0的中断向量号
            break;
            case 1:
        ...
        /* 初始化为0，目的是向硬盘控制器请求数据后，硬盘驱动sema_down此信号阻塞线程，
        * 直到硬盘完成后通过发中断，由中断处理程序将此信号量sema_up唤醒线程 */
