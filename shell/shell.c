...
char final_path[MAX_PATH_LEN]={0};   // 清除路径缓冲
...
static void readline(char* buf,int32_t count){
    ...
    while (read(stdin_no,pos,1)!=-1 && (pos-buf)<count){   // 直至回车符停止
		switch (*pos){
			/* 找到回车或换行符后认为键入命令结束 */
			case '\n':
			case '\r':
...
static void cmd_execute(uint32_t argc,char** argv){
	...
	int32_t pid=fork();
        if (pid){      // 父进程
            int32_t status;
            int32_t child_pid=wait(&status);   // 子进程若没有执行exit()，则my_shell会被阻塞
			if (child_pid==-1){
...
char* argv[MAX_ARG_NR]={NULL};   // argv必须为全局变量
int32_t argc=-1;
...
void my_shell(void){
    ...
    while (1){
		...
		* cmd1的标准输出和cmdn的标准输入需要单独处理 */
			...
			int32_t fd[2]={-1};
			pipe(fd);
			/* 将标准输出重定向到fd[1]，使后续的输出信息重定向到内核环形缓冲区 */
			fd_redirect(1,fd[1]);
